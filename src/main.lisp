...
(defmacro build-board (dim)
  `(make-board :board (make-array '(,dim ,dim) :initial-element 'empty)))
...
(defun board-spot-check (board spot)
  (let
    ...
    (if
      (and (in-closed-range (first spot) indexrange) (in-closed-range (second spot) indexrange))
      T
      '())))
...
;; Return a shallow copy of an array. Code courtesy of http://jtra.cz/stuff/lisp/sclr/copy-seq.html .
      ...
      (play-game
        ...
        (build-state-list
          (if
            ...
            (list (board-apply-move board (funcall (player-select-move current-player) state current-player)) (circular-queue-advance players))))))))
